...
int add_func(int argc, char *argv[])
{
    ...
    sdom.vcpuid = vcpu_id;
    if ( xc_sched_rtds_domain_add_dedvcpu(xch, dom_id, &sdom) )
    {
...
int remove_func(int argc, char *argv[])
{
    ...
    sdom.vcpuid = vcpu_id;
    if ( xc_sched_rtds_domain_remove_dedvcpu(xch, dom_id, &sdom) )
    {
        ERROR("Could not set vcpu %d of dom %d as non-dedicated VCPU\n",
              vcpu_id, dom_id);
        return -1;
    }

    printf("vcpu %d of dom %d has been set as non-dedicated VCPU\n",
           vcpu_id, dom_id);
