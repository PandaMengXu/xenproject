...
void repl_timer_fn(void *unused);
...
struct rt_private {
    ...
    struct list_head runq;     /* ordered list of runnable vcpus based on deadline*/
    struct list_head replq;    /* ordered list of all vcpus based on replenish time*/
    struct timer repl_timer;   /* timer to replenish vcpus' budget */
    struct rt_vcpu *flag_vcpu; /* position of the first depleted vcpu (zero budget) */
    s_time_t next_repl;        /* next earliest budget replenish time */
...
struct rt_vcpu {
    ...
    struct list_head replq_elem;  /* on the replenish queue list */
    ...
    long cur_budget;             /* current budget in nanosecond */
    s_time_t cur_deadline;      /* current deadline for EDF */
    s_time_t next_repl;         /* next budget replenish time */
...
static inline struct list_head *REPLQ(const struct scheduler *_ops)
{
    return &RT_PRIV(_ops)->replq;
}
...
/*
 * ReplenishQueue helper functions
 */
static int
...
static inline void
__replq_remove(struct rt_vcpu *svc)
{
...
/*
 * Insert a vcpu into the ReplenishQueue based on vcpu' budget replenish time: 
 * The vcpu svc to be inserted will be inserted just before the very first 
 * vcpu iter_svc in the Runqueue whose budget replenish time is equal 
 * or larger than svc's deadline.
 */
static void
__replq_insert(const struct scheduler *ops, struct rt_vcpu *svc)
{
    struct rt_private *prv = RT_PRIV(ops);
    struct list_head *replq = REPLQ(ops);
    struct list_head *iter;
    struct rt_vcpu *head_svc;
    spinlock_t *schedule_lock;
    
    /* TODO: use a lock for replenish queue only */
    schedule_lock = per_cpu(schedule_data, svc->vcpu->processor).schedule_lock;
    ASSERT( spin_is_locked(schedule_lock) );
    ...
    {
        struct rt_vcpu *iter_svc = __replq_elem(iter);
        if ( svc->next_repl <= iter_svc->next_repl )
                break;
     }
    list_add_tail(&svc->replq_elem, iter);

    /* check if earliest replenish time can be earlier */
    head_svc = __replq_elem(replq->next);
    if ( head_svc->next_repl < prv->next_repl )
    {
        /* reset the timer to head_svc->next_repl */
        set_timer(&prv->repl_timer, head_svc->next_repl);
        prv->next_repl = head_svc->next_repl;
    }
}
...
rt_init(struct scheduler *ops)
{
    ...
    init_timer(&prv->repl_timer, repl_timer_fn, NULL, 0);
    prv->next_repl = 0;
...
rt_alloc_vdata(const struct scheduler *ops, struct vcpu *vc, void *dd)
{
    ...
    svc->cur_budget = svc->budget*1000; /* counting in nanosecond level */
    
    svc->next_repl = svc->cur_deadline;
...
__repl_update(const struct scheduler *ops, s_time_t now)
{
    ...
    list_for_each_safe(iter, tmp, replq)
    {
        ...
        diff = now - svc->next_repl;
        if ( diff >= 0 ) 
        {
            count = (diff/MICROSECS(svc->period)) + 1;
            svc->next_repl += count * MICROSECS(svc->period);
            __replq_remove(svc);
...
rt_schedule(const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
    ...
    /* update budget if earliest replenish time arrives */
    if ( now >= prv->next_repl )
...
/* force a run of schedule function when replenish time arrives */
void repl_timer_fn(void *unused)
{
